...
this.latticeCells = null;
var bmChains = this.acr.getFilter ("BMCHAINS");
var fixBMChains = (bmChains == null ? -1 : bmChains.length < 2 ? 0 : bmChains.charCodeAt (1) - 48);
...
}var minChain = atoms[this.firstAtom].chainID;
var maxChain = 0;
for (var iAtom = this.firstAtom; iAtom < atomMax; iAtom++) {
atoms[iAtom].bsSymmetry = JU.BSUtil.newAndSetBit (0);
...
var delta = (fixBMChains > 0 ? maxChain - minChain - 66 : 0);
atoms = this.asc.atoms;
for (var i = atomMax, n = this.asc.ac; i < n; i++) {
var ic = atoms[i].chainID;
var isym = atoms[i].bsSymmetry.nextSetBit (0) + 1;
var ch = (delta == 0 ? this.acr.vwr.getChainIDStr (ic) + isym : "" + String.fromCharCode (((ic + delta + isym) % 26) + 65));
atoms[i].chainID = this.acr.vwr.getChainID (ch, true);
}
