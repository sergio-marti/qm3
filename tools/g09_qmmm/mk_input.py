...
for fn in [ "qmmm.sel", "qmmm.crd", "qmmm.psf", "qmmm.vdw" ]:
...
while( l[0] == "*" ):
	l = f.readline()
for l in f:
	labl.append( l[16:20].strip().upper() )
	coor += [ float( l[20:30] ), float( l[30:40] ), float( l[40:50] ) ]
f.close()
...
f = open( "qmmm.vdw", "rt" )
for i,j,k in re.compile( "([A-Z0-9]+)[\ ]+([0-9\.]+)[\ ]+([0-9\.]+)" ).findall( f.read().upper() ):
