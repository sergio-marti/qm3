...
for ai, aj, ak, al in DIE:
    pijkl = re.compile( "^%s[\ ]+%s[\ ]+%s[\ ]+%s[\ ]+([0-9\-\.]+)[\ ]+([1-6])[\ ]+([0-9\.]+)"%( TBL[ai], TBL[aj], TBL[ak], TBL[al] ) )
    plkji = re.compile( "^%s[\ ]+%s[\ ]+%s[\ ]+%s[\ ]+([0-9\-\.]+)[\ ]+([1-6])[\ ]+([0-9\.]+)"%( TBL[al], TBL[ak], TBL[aj], TBL[ai] ) )
    pxjkx = re.compile( "^X[\ ]+%s[\ ]+%s[\ ]+X[\ ]+([0-9\-\.]+)[\ ]+([1-6])[\ ]+([0-9\.]+)"%( TBL[aj], TBL[ak] ) )
    pxkjx = re.compile( "^X[\ ]+%s[\ ]+%s[\ ]+X[\ ]+([0-9\-\.]+)[\ ]+([1-6])[\ ]+([0-9\.]+)"%( TBL[ak], TBL[aj] ) )
    for l in b:
        t = []
        if( pijkl.match( l ) ):
            t = l.strip().split()
