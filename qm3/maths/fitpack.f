...
c* Source for module FITPACK from package FITPACK.
c* Retrieved from NETLIB on Tue Jun  8 10:41:59 1999.
c* =====================================================================

c  Implementation of FITPACK by MLM.  All routines have been converted
c  to double precision via am implicit statement.  All functions
c  have been renamed double precision functions.  Also, all 
c  occurrences of abs, float, sin,
c  cos, and exp have been converted to double precision forms.
c  Constants in data statements have been given "d" exponents.
c  Only the snhcsh_fpck subroutine of the highest precision has been
c  retained, i.e. the lower precision one has been omitted.
c  Finally, to make this work with gnu fortran, all occurrences of ifix
c  have been replaced by int.
...
c
      sigmap = dabs(sigma)*dfloat(n-1)/(x(n)-x(1))
c
...
c
      implicit real*8 (a-h,o-z)
      integer n,isw,ierr
      real*8 x(n),y(n),d(n),s,eps,ys(n),ysp(n),sigma,temp(n,9)
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
     ...
     *             ierr)
      ...
      double precision function curv2 (t,n,x,y,yp,sigma)
c
      implicit real*8 (a-h,o-z)
      integer n
...
c
c this function references package modules intrvl_fpck and
c snhcsh_fpck.
c
c-----------------------------------------------------------
c
...
c
    ...
    1 sigdel = sigmap*dels
      call snhcsh_fpck (ss,dummy,sigdel,-1)
      call snhcsh_fpck (s1,dummy,sigmap*del1,-1)
      call snhcsh_fpck (s2,dummy,sigmap*del2,-1)
      curv2 = sum+(yp(i)*del1*(s1-ss)+yp(im1)*del2*(s2-ss))/
     *            (sigdel*sigmap*(1.+ss))
      return
      end
      double precision function curvd (t,n,x,y,yp,sigma)
c
      implicit real*8 (a-h,o-z)
      integer n
      real*8 t,x(n),y(n),yp(n),sigma
...
c
    ...
    1 sigdel = sigmap*dels
      call snhcsh_fpck (ss,dummy,sigdel,-1)
      call snhcsh_fpck (dummy,c1,sigmap*del1,1)
      call snhcsh_fpck (dummy,c2,sigmap*del2,1)
      curvd = sum+(yp(i)*(c1-ss)-yp(im1)*(c2-ss))/
     *        (sigdel*sigmap*(1.+ss))
      return
      end
      double precision function curvi (xl,xu,n,x,y,yp,sigma)
c
      implicit real*8 (a-h,o-z)
      integer n
...
c
    1 ilm1 = intrvl_fpck (xxl,x,n)
      il = ilm1+1
      ium1 = intrvl_fpck (xxu,x,n)
      iu = ium1+1
      if (il .eq. iu) go to 8
c
...
c
      ...
      call snhcsh_fpck (dummy,c1,sigmap*del1,2)
      call snhcsh_fpck (dummy,c2,sigmap*del2,2)
      call snhcsh_fpck (ss,cs,sigmap*dels,3)
...
c
    8 delu1 = xxu-x(ium1)
      ...
      call snhcsh_fpck (dummy,cu1,sigmap*delu1,2)
      call snhcsh_fpck (dummy,cu2,sigmap*delu2,2)
      call snhcsh_fpck (dummy,cl1,sigmap*dell1,2)
      call snhcsh_fpck (dummy,cl2,sigmap*dell2,2)
      call snhcsh_fpck (ss,dummy,sigmap*dels,-1)
...
c
      implicit real*8 (a-h,o-z)
      integer n,ierr
      real*8 x(n),y(n),p,yp(n),temp(1),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
      implicit real*8 (a-h,o-z)
      integer n,isw,ierr
      real*8 x(n),y(n),p,d(n),s,eps,ys(n),ysp(n),sigma,
     *     temp(n,11)
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c
     ...
     *             temp(1,9),temp(1,10),temp(1,11),ierr)
      ...
      double precision function curvp2 (t,n,x,y,p,yp,sigma)
c
      implicit real*8 (a-h,o-z)
      integer n
      real*8 t,x(n),y(n),p,yp(n),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
     ...
     *            (sigdel*sigmap*(1.+ss))
      ...
      double precision function curvpi (xl,xu,n,x,y,p,yp,sigma)
c
      implicit real*8 (a-h,o-z)
      integer n
      real*8 xl,xu,x(n),y(n),p,yp(n),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
      ...
      ideltp = uper-lper
      bdy = dfloat(ideltp)*(xxu-xxl) .lt. 0.
...
c
   ...
   20 so = s1+s2+s6+s7
      si = s3+s4+s5+s8
      if (bdy) go to 21
      curvpi = dfloat(ideltp)*(so+si)+dfloat(isign)*si
      return
   21 curvpi = dfloat(ideltp)*(so+si)+dfloat(isign)*so
      return
      end
...
c
      implicit real*8 (a-h,o-z)
      integer n,islpsw,ierr
      real*8 x(n),y(n),slp1,slpn,xp(n),yp(n),temp(n),s(n),
     *     sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c denormalize tension factor
c
      sigmap = dabs(sigma)*dfloat(n-1)/s(n)
c
c approximate end slopes
c
...
c
      implicit real*8 (a-h,o-z)
      integer n
      real*8 t,xs,ys,x(n),y(n),xp(n),yp(n),s(n),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c none of the input parameters are altered.
c
c this subroutine references package modules intrvl_fpck and
c snhcsh_fpck.
c
c-----------------------------------------------------------
c
...
c
      implicit real*8 (a-h,o-z)
      integer n
      real*8 t,xs,ys,xst,yst,xstt,ystt,x(n),y(n),xp(n),yp(n),
     *     s(n),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c
    ...
    1 sigdel = sigmap*dels
      call snhcsh_fpck (ss,dummy,sigdel,-1)
      call snhcsh_fpck (s1,co1,sigmap*del1,0)
      call snhcsh_fpck (s2,co2,sigmap*del2,0)
      d = sigdel*sigmap*(1.+ss)
      c1 = del1*(s1-ss)/d
      c2 = del2*(s2-ss)/d
      ct1 = (co1-ss)*s(n)/d
...
c
      implicit real*8 (a-h,o-z)
      integer n,ierr
      real*8 x(n),y(n),xp(n),yp(n),temp(1),s(n),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c denormalize tension factor
c
      sigmap = dabs(sigma)*dfloat(n)/s(n)
c
...
c
      ...
      tn = s(n)*tn+s(1)
      im1 = n
      if (tn .lt. s(n)) im1 = intrvl_fpck(tn,s,n)
      i = im1+1
      if (i .gt. n) i = 1
c
c denormalize tension factor
...
c
      ...
      real*8 x(m),y(n),z(iz,n),zx1(n),zxm(n),zy1(m),zyn(m),
...
c
      implicit real*8 (a-h,o-z)
      integer m,n,iz
      real*8 xx,yy,x(m),y(n),z(iz,n),zp(m,n,3),sigma
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
      jm1 = intrvl_fpck (yy,y,n)
      j = jm1+1
c
c determine x interval
c
      im1 = intrvl_fpck (xx,x,m)
      i = im1+1
      del1 = yy-y(jm1)
      del2 = y(j)-yy
      dels = y(j)-y(jm1)
...
c
     ...
     *              zp(i,j-1,3),zp(i,j,3))
      go to 2
    1 call snhcsh_fpck (sinhm1,dummy,sigmay*del1,-1)
      call snhcsh_fpck (sinhm2,dummy,sigmay*del2,-1)
      call snhcsh_fpck (sinhms,dummy,sigmay*dels,-1)
...
c
    2 del1 = xx-x(im1)
      ...
      return
    3 call snhcsh_fpck (sinhm1,dummy,sigmax*del1,-1)
      call snhcsh_fpck (sinhm2,dummy,sigmax*del2,-1)
      call snhcsh_fpck (sinhms,dummy,sigmax*dels,-1)
...
c none of the input parameters are altered.
c
c this subroutine references package module snhcsh_fpck.
c
c-----------------------------------------------------------
c
      if (n .eq. 2) go to 2
...
c tension .ne. 0.
c
    1 call snhcsh_fpck (dummy,coshm1,sigma*del1,1)
      call snhcsh_fpck (dummy,coshm2,sigma*del2,1)
      delp = sigma*(del2+del1)/2.
      delm = sigma*(del2-del1)/2.
      call snhcsh_fpck (sinhmp,dummy,delp,-1)
      call snhcsh_fpck (sinhmm,dummy,delm,-1)
...
c
      implicit real*8 (a-h,o-z)
      integer n,isw,ierr
      real*8 x(n),y(n),p,d(n),s,eps,ys(n),ysp(n),sigma,td(n),
...
c
     ...
     *     rsd2(n),v(n)
      ...
      real*8 alpha, alphap, beta, betap, betapp, delxi, delxi1, 
     #       delyi, delyi1, di, dim1, f, g, h, hdi, hdim1, hsd1p, p,
     #       rdim1, rsd1i, rsd2i, sigmap, sl, step, su, sum, tui, 
     #       wi, wim1, wim2, yspim2
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c
   20 ierr = 5
      ...
      integer function intrvl_fpck (t,x,n)
c
      implicit real*8 (a-h,o-z)
      integer n
      real*8 t,x(n)
...
c on output--
c
c   intrvl_fpck returns an integer i such that
c
c          i =  1       if         e   t .lt. x(2)  ,
...
c
    1 i = (il+ih)/2
      ...
      subroutine snhcsh_fpck (sinhm,coshm,x,isw)
c
      implicit real*8 (a-h,o-z)
      integer isw
      real*8 sinhm,coshm,x
      real*8 ax, cp1, cp2, cp3, cp4, cp5, cq0, cq1, cq2, expx, 
     #       sp11, sp12, sp13, sp14, sp21, sp22, sp23, sp24, sp25, 
     #       sp31, sp32, sp33, sp34, sp35, sp41, sp42, sp43, sp44, 
     #       sp45, sq10, sq11, sq12, sq20, sq21, sq22, sq30, sq31, 
     #       sq32, sq40, sq41, sq42, xs, zp1, zp2, zp3, zp4, zq0, 
     #       zq1, zq2
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
...
c-----------------------------------------------------------
c
      data sp14/.227581660976348d-7/,
     *     sp13/.612189863171694d-5/,
     *     sp12/.715314759211209d-3/,
     *     sp11/.398088289992973d-1/,
     *     sq12/.206382701413725d-3/,
     *     sq11/-.611470260009508d-1/,
     *     sq10/.599999999999986d+1/
      data sp25/.129094158037272d-9/,
     *     sp24/.473731823101666d-7/,
     *     sp23/.849213455598455d-5/,
     *     sp22/.833264803327242d-3/,
     *     sp21/.425024142813226d-1/,
     *     sq22/.106008515744821d-3/,
     *     sq21/-.449855169512505d-1/,
     *     sq20/.600000000268619d+1/
      data sp35/.155193945864942d-9/,
     *     sp34/.511529451668737d-7/,
     *     sp33/.884775635776784d-5/,
     *     sp32/.850447617691392d-3/,
     *     sp31/.428888148791777d-1/,
     *     sq32/.933128831061610d-4/,
     *     sq31/-.426677570538507d-1/,
     *     sq30/.600000145086489d+1/
      data sp45/.188070632058331d-9/,
     *     sp44/.545792817714192d-7/,
     *     sp43/.920119535795222d-5/,
     *     sp42/.866559391672985d-3/,
     *     sp41/.432535234960858d-1/,
     *     sq42/.824891748820670d-4/,
     *     sq41/-.404938841672262d-1/,
     *     sq40/.600005006283834d+1/
      data cp5/.552200614584744d-9/,
     *     cp4/.181666923620944d-6/,
     *     cp3/.270540125846525d-4/,
     *     cp2/.206270719503934d-2/,
     *     cp1/.744437205569040d-1/,
     *     cq2/.514609638642689d-4/,
     *     cq1/-.177792255528382d-1/,
     *     cq0/.200000000000000d+1/
      data zp4/.664418805876835d-8/,
     *     zp3/.218274535686385d-5/,
     *     zp2/.324851059327161d-3/,
     *     zp1/.244515150174258d-1/,
     *     zq2/.616165782306621d-3/,
     *     zq1/-.213163639579425d0/,
...
c
      ...
      real*8 coshm, denom, sigdel, sinhm
c
c                                 coded by alan kaylor cline
c                           from fitpack -- january 26, 1987
c                        a curve and surface fitting package
...
c
      ...
      return
    1 sigdel = sigma*del
      call snhcsh_fpck (sinhm,coshm,sigdel,0)
